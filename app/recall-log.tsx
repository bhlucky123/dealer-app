...
const RecallLogScreen = () => {
  ...
  const items: RecallLog[] = useMemo(() => {
    if (Array.isArray(rawData)) return rawData;
    const r = (rawData as any)?.results;
    return Array.isArray(r) ? r : [];
  }, [rawData]);

  const totalRecalled = items.reduce((s, i) => s + (i?.recalled_count ?? 0), 0);
    ...
    <SafeAreaView className="flex-1 bg-white" edges={["bottom"]}>
      <View className="flex-1 p-4">
        ...
        <View className="mb-3 flex-row items-center gap-2">
          <View className="flex-1 bg-orange-50 border border-orange-100 rounded-xl px-3 py-2.5">
            <Text className="text-[10px] text-orange-500 font-semibold uppercase">
              Draw
            </Text>
            <Text
              className="text-orange-900 font-bold text-sm mt-0.5"
              numberOfLines={1}
            >
              {selectedDraw?.name || "—"}
            </Text>
        ...
        {!selectedDraw?.id ? (
          ...
          <View className="flex-1 justify-center items-center">
            ...
            <Text className="mt-3 text-gray-600">Loading recalls...</Text>
          </View>
        ) : error ? (
          <View className="flex-1 justify-center items-center">
...
export default RecallLogScreen;
