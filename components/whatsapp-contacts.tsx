...
export function contactErrors(rows: WhatsAppContact[]): string[] {
  const phones = rows.map(row => normalizePhone(row.phone_number));
...
export function initialContacts(user: { whatsapp_contacts?: WhatsAppContact[]; whatsapp_numbers?: string[]; phone_number?: string; whatsapp_result_subscribed?: boolean }): WhatsAppContact[] {
  if (user.whatsapp_contacts) return user.whatsapp_contacts;
  const phones = [...new Set([user.phone_number, ...(user.whatsapp_numbers || [])].filter(Boolean))] as string[];
  return phones.map((phone, index) => ({ phone_number: phone, is_active: true, receive_results: index === 0 && !!user.whatsapp_result_subscribed }));
}

export function WhatsAppContacts({ value, onChange, serverErrors }: { value: WhatsAppContact[]; onChange: (rows: WhatsAppContact[]) => void; serverErrors?: any }) {
  if (typeof serverErrors === "string") {
    try { serverErrors = JSON.parse(serverErrors); } catch { /* Show a general field error below. */ }
  }
  const errors = contactErrors(value);
  const change = (index: number, patch: Partial<WhatsAppContact>) => onChange(value.map((row, i) => i === index ? { ...row, ...patch } : row));
  return <View style={{ gap: 12, marginVertical: 16 }}>
    ...
    {value.map((row, index) => <View key={index} style={{ padding: 14, borderWidth: 1, borderColor: "#cbd5e1", borderRadius: 12, gap: 10 }}>
      ...
      <Pressable accessibilityRole="button" onPress={() => onChange(value.filter((_, i) => i !== index))}><Text style={{ color: "#b91c1c", paddingVertical: 8 }}>Remove number</Text></Pressable>
    </View>)}
    <Pressable accessibilityRole="button" onPress={() => onChange([...value, { phone_number: "", receive_results: false, is_active: true }])}><Text style={{ color: "#0369a1", padding: 12 }}>+ Add number</Text></Pressable>
